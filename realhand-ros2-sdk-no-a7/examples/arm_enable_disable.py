#!/usr/bin/env python3
"""Enable, disable, reset errors, or resume after e-stop for a RealHand arm over ROS2 topics."""
...
class ArmActionPublisher(Node):
    def __init__(self, side: str) -> None:
        super().__init__("realhand_example_arm_enable_disable")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "action",
        choices=["enable", "disable", "reset_error", "resume_from_emergency_stop"],
    )
...
def main() -> None:
    args = parse_args()
    rclpy.init()
    node = ArmActionPublisher(args.side)
    try:
        if not wait_for_subscribers(node, node.publisher, args.wait_timeout):
            raise SystemExit(f"no subscriber found on {node.topic}")
        node.publish_action(args.action)
        node.get_logger().info(f"Published {args.action} to {node.topic}")
