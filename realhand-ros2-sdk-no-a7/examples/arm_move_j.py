#!/usr/bin/env python3
"""Move a RealHand arm with move_j through ROS2 command_json."""
...
class ArmMoveJPublisher(Node):
    def __init__(self, side: str) -> None:
        super().__init__("realhand_example_arm_move_j")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--velocities",
        type=parse_csv_floats,
        help="Optional seven joint velocity limits/targets.",
    )
    parser.add_argument(
        "--accelerations",
        type=parse_csv_floats,
        help="Optional seven joint accelerations.",
    )
    parser.add_argument(
        "--blocking",
        action="store_true",
        help="Ask the backend to block inside the SDK move_j call.",
    )
    parser.add_argument(
        "--wait-timeout",
        type=float,
...
def main() -> None:
    ...
    payload: dict[str, object] = {"joints": args.joints, "blocking": args.blocking}
    ...
    rclpy.init()
    node = ArmMoveJPublisher(args.side)
    try:
        ...
        node.get_logger().info(f"Published move_j payload to {node.topic}: {payload}")
